...
# scale sky to 1 (follows camera)
# take 1 and divide by number of layers 

const BG_FILES: Dictionary = {
	# preload list of files is in specific order, for each level
	1: [
...
func add_backgrounds() -> void:
	...
	for index in range(files_list.size()):
		...
		if index == 0:
			# reason for using 1 is... 
			# this is the sky layer
			add_layer(bg_file, 1)
		else:
